...
public class tennis extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        liste = view.findViewById(R.id.recview2);
        liste.setLayoutManager(new LinearLayoutManager(getContext()));

        ref = FirebaseDatabase.getInstance().getReference().child("Tennis");
    ...
    private void processsearch(String s) {
        FirebaseRecyclerOptions<Sport> options =
                new FirebaseRecyclerOptions.Builder<Sport>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Tennis").orderByChild("titre").startAt(s).endAt(s + "\uf8ff"), Sport.class)
                        .build();

...
public class football extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        liste = view.findViewById(R.id.recview);


        liste.setLayoutManager(new LinearLayoutManager(getContext()));

        ref = FirebaseDatabase.getInstance().getReference().child("Football");
    ...
    private void processsearch(String s) {
        FirebaseRecyclerOptions<Sport> options =
                new FirebaseRecyclerOptions.Builder<Sport>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Football").orderByChild("titre").startAfter(s).endAt(s + "\uf8ff"), Sport.class)
                        .build();

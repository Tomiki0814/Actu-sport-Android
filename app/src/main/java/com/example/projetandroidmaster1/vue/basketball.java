...
public class basketball extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_basketball,container,false);
        liste = view.findViewById(R.id.recview3);
        liste.setLayoutManager(new LinearLayoutManager(getContext()));

        ref = FirebaseDatabase.getInstance().getReference().child("Basket");
    ...
    private void processsearch(String s) {
        FirebaseRecyclerOptions<Sport> options =
                new FirebaseRecyclerOptions.Builder<Sport>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Basket").orderByChild("titre").startAt(s).endAt(s + "\uf8ff"), Sport.class)
                        .build();
